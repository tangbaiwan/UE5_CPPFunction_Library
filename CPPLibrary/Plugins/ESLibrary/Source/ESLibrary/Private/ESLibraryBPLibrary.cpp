...
FRotator UESLibraryBPLibrary::GridSnapRotation(FRotator Rotator, float GridDeg)
{
	...
	FVector forwardResult;
	{
		...
		{
			TArray<double> maxHelper = { FMath::Abs(forward.X), FMath::Abs(forward.Y), FMath::Abs(forward.Z) };
...
// 			{
// 				UE_LOG(LogESLibraryBPLibrary, Display, TEXT("Started saving checkpoint at recording time: [%f]"), World->GetDemoNetDriver()->GetDemoCurrentTime());
// 				World->GetDemoNetDriver()::SavePropertyState();
// 			}
// 			else
// 			{
	...
	if (SaveSystem && (SlotName.Len() > 0))
	{
		...
		if (bSuccess)
		{
			...
			if (SaveGameClass != NULL)
			{
				...
				FName CompressionFormat = "Text";
				FArchiveLoadCompressedProxy Decompressor =
					FArchiveLoadCompressedProxy(CompressedData, CompressionFormat,ECompressionFlags::COMPRESS_ZLIB);
